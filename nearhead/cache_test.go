...
func createTestData(t *testing.T) evm.BlockData {
...
func createTestDataBatch(t *testing.T, size int) []evm.BlockData {
	datas := make([]evm.BlockData, 0, size)
	for i := 0; i < size; i++ {
...
func TestEthCache_Pop(t *testing.T) {
	...
	popped := cache.Pop(uint64(120177555 + batchBlocks))
	...
	// pop one block
	popped = cache.Pop(uint64(120177555 + batchBlocks - 1))
	...
	// pop two blocks
	popped = cache.Pop(uint64(120177555 + batchBlocks - 3))
